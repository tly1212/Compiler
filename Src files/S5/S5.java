...
class S5
{
  public static void main(String[] args) throws
                                             IOException
  {
    System.out.println("S5 compiler written by ...");
    ...
    outFile.println("; from S5 compiler written by ...");
...
class S5TokenMgr implements S5Constants
{
  ...
  //-----------------------------------------
  public S5TokenMgr(Scanner inFile,
  ...
  private void getNextChar()
  {
    ...
    // in S5, test for single-line comment goes here
    if (currentChar == '/'
    && inputLine.charAt(currentColumnNumber) == '/'
    && !keepComment) {
      currentChar = '\n';
...
class S5Parser implements S5Constants
{
  ...
  private ArrayList breakLabel;
  
  //-----------------------------------------
  public S5Parser(S5SymTab st, S5TokenMgr tm,
                                           S5CodeGen cg)
  {
    this.st = st;
    this.tm = tm;
  ...
  private void functionDefinition()
  {
    ...
    consume(LEFTPAREN);
    if (currentToken.kind == INT) parameterList();
    consume(RIGHTPAREN);
    consume(LEFTBRACE);
  ...
  private void statement()
  {
    try {
      ...
      {
        case ID:
          if (getToken(2).kind == ASSIGN) assignmentStatement();
          else if (getToken(2).kind == LEFTPAREN) functionCall();
